...
def approval_program():
    ...
    on_lock_tx_init_storage = Seq(
        ...
        App.localPut(Txn.sender(), Bytes("manager"), on_lock_appcall_tx_manager)
    )

    on_lock_tx_config = And(
        Gtxn[4].sender() != Gtxn[3].sender(),
        Gtxn[4].sender() != Gtxn[2].sender(),
        Gtxn[4].sender() == Gtxn[1].sender(),
        Gtxn[4].sender() == Gtxn[0].sender(),
        Gtxn[4].type_enum() == TxnType.AssetConfig,
        Gtxn[4].xfer_asset() != Txn.assets[0], # More specific in signature
        # Signature must be the new asset manager
        Or(
             Gtxn[4].config_asset_manager() == Gtxn[2].sender()
        ),
        # Signature must be the new clawback or 0
        Or(
            Gtxn[4].config_asset_clawback() == Gtxn[2].sender(),
            Gtxn[4].config_asset_clawback() == Global.zero_address()
        ),
        # Signature must be the freeze or 0
        Or(
            Gtxn[4].config_asset_freeze() == Gtxn[2].sender(),
    ...
    on_create = Seq(
        ...
        App.globalPut(Bytes("tinylock_locker_address"),
                      AppParam.address(Txn.applications[1])
                      ),
        Approve()
    )
