...
def approval_program(
    ...
    on_lock_tx_config = And(
        # Signature must be the asset manager
        Or(
            Gtxn[4].config_asset_manager() == Txn.sender()
        ),
        # Signature must be the new clawback or 0
        Or(
            Gtxn[4].config_asset_clawback() == Txn.sender(),
            Gtxn[4].config_asset_clawback() == Global.zero_address()
        ),
        # Signature must be the freeze or 0
        Or(
            Gtxn[4].config_asset_freeze() == Txn.sender(),
            Gtxn[4].config_asset_freeze() == Global.zero_address()
        )
    )

    gtx_lock_perm_to_sig = And(
        Gtxn[4].type_enum() == TxnType.AssetConfig,  # Transfer
        Gtxn[4].sender() != Txn.sender(),  # Signature is not the sender
        # Sender is Signature creator
        Gtxn[4].sender() == Addr(TMPL_LOCKER_ADDRESS),
        Gtxn[4].config_asset_manager() == Txn.sender(),  # Signature is receiver
        # Must be the token that is being locked
        Gtxn[4].xfer_asset() == TMPL_ASSET_ID,
        on_lock_tx_config()
    ...
    on_unlock_tx_config = And(
        # Signature must be the asset manager
        Or(
            Gtxn[2].config_asset_manager() == Addr(TMPL_LOCKER_ADDRESS)
        ),
        # Signature must be the new clawback or 0
        Or(
            Gtxn[2].config_asset_clawback() == Addr(TMPL_LOCKER_ADDRESS),
            Gtxn[2].config_asset_clawback() == Global.zero_address()
        ),
        # Signature must be the freeze or 0
        Or(
            Gtxn[2].config_asset_freeze() == Addr(TMPL_LOCKER_ADDRESS),
            Gtxn[2].config_asset_freeze() == Global.zero_address()
        )
    )

    gtx_unlock_asset_to_locker = And(
        Gtxn[2].type_enum() == TxnType.AssetConfig,  # Transfer
        Gtxn[2].sender() == Txn.sender(),  # Signature is the sender
        # Must be the token that is being locked
        Gtxn[2].xfer_asset() == TMPL_ASSET_ID,
        on_unlock_tx_config
    )
